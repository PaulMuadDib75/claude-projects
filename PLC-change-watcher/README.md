...
- Starts automatically at login via Task Scheduler
...
| Machine | Script Location |
|---|---|
| Development | `C:\claude\projects\plc-change-watcher\` |
| Production | `C:\Python\PLC-change-watcher\` |

After any code change, copy the updated script to the production machine:

```powershell
Copy-Item "C:\claude\projects\PLC-change-watcher\plc_change_watcher.py" "C:\Python\PLC-change-watcher\plc_change_watcher.py"
```
...
| Equipment Name / Number | The equipment affected by the change |
...
| 2 | Processor / Program | Read-only | Auto-filled from filename |
| 3 | Routine | Text entry | Required — highlights yellow if empty |
| 4 | Equipment Name / Number | Text entry | Required — highlights yellow if empty |
| 5 | Rung | Text entry | Required — highlights yellow if empty |
| 6 | Description of Change | Multi-line text | Required |
| 7 | Reason for Change | Text entry | Required |
| 8 | Authorized By | Text entry | Required |
| 9 | Date / Time | Read-only | Auto-filled |
...
| Two popups appear on one save | RSLogix backup files triggering watcher — confirm extension filter uses `Path(path).suffix != ".ACD"` without `.upper()` |
...
| CSV has no header row | Confirm `ensure_csv_headers()` is called inside `on_submit()` before the CSV write |
| CSV columns in wrong order | Delete `plc_change_log.csv` and restart the script to regenerate headers |
| Changes not showing on production | Copy updated script from dev machine to `C:\Python\PLC-change-watcher\` |
